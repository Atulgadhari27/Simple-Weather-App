...
// This function displays some default city weathers in an carousul
defaultWeatherResults();

// Events listners to trigger searchFunction, after onCLick or keyup event
...
enterKeySearch.addEventListener("keyup", function (e){
    if(e.key == "Enter"){
...
function searchFunction(link, setFunction){
    ...
    const apikey = "&appid=ee56579adb94c1bc09ccc1b5b40a6d06";
    ...
    xmlRequest.onload = function(){
        if(this.status === 200){
            setFunction(xmlRequest.responseText);
            onSuccessfulSearch();
        }
...
// Error function handles errors like 404 and emptyInput error
function Error(type){
    if(type === '404'){
...
// this function fetches the weather results as per the user input
        ...
        <div class="row">
            <div id="middle" class="col-11 col-md-8 currentWeatherCard">
                ...
                <div class="row weatherMain">
                    <!-- Weather Icon-->
                    <div class="col col-6">
                        <img src="images/forecast-icons/${weatherData.weather[0].icon}.png" alt="Weather Icon">
                    </div>
                ...
                <div class="row sunTimes">
                    ...
                    <div class="col col-5">
                        <h2><img src="images/sun-rise-time.png" alt="Wind"> ${sunTimes(weatherData.sys.sunrise, weatherData.timezone)}</h2>
                    </div>
                    <!-- Sun Set Icon and Time-->
                    <div class="col col-5">
                        <h2><img src="images/sun-set-time.png" alt="Wind"> ${sunTimes(weatherData.sys.sunset, weatherData.timezone)}</h2>
                    </div>
                </div>
                ...
                <div class="row weatherForecasts">
                    <!-- Wind Icon and Speed-->
                    <div class="col col-4">
                        <h2><img src="images/wind-speed.png" alt="Wind"> ${msToKMH(weatherData.wind.speed)} <span class="unit">km/h</span></h2>
                    </div>
                    <!-- Humidity Icon and Humidity-->
                    <div class="col col-4">
                        <h2><img src="images/humidity.png" alt="Humidity"> ${weatherData.main.humidity}<span class="unit">&#37;</span></h2>
                    </div>
                    <!-- Rain Icon and Precipitation -->
                    <div class="col col-4">
                        <h2><img src="images/precipitation.png" alt="Rain"> ${ifRaining(weatherData.rain)}<span class="unit">mm</span></h2>
                    </div>
                </div>
...
// this function convers wind speed from m/s to km/h
function msToKMH(wind){
    let msToKMHCal = (wind * 3.6).toFixed(1);
    return msToKMHCal;
}
...
// this function adds carousel to the page with weather data of some important cities
 function defaultWeatherResults(){
    const apikey = "&appid=ee56579adb94c1bc09ccc1b5b40a6d06";
    let units = "&units=metric";
    let link = "https://api.openweathermap.org/data/2.5/weather?q=";

    let cities = ["Tokyo", "Moscow", "Delhi", "Cairo", "London", "Paris", "Mexico", "Ottawa"];

    let defaultWeatherOutput = "";

    // this creates the structure for the carousel
    ...
    defaultWeatherOutput += `
        ...
        document.getElementById("carouselTarget").innerHTML = defaultWeatherOutput;
        

    
    // this fetches the data of some important cities
    for(let i = 0; i < cities.length; i++){

        let url = link + cities[i] + units + apikey;

        let xml = new XMLHttpRequest();
        xml.open("GET", url, true);
        xml.onload = function(){
            if(this.status === 200){
                let data = setWeather(xml.responseText);
                document.getElementById("myDIV").innerHTML += data;

                // this is required because the onload function is an async function
                // and sliderInit is needed to be called onl when all data is fetched for all cities
                // hence this if statement is used
...
// this function takes one parameter which is the data fetched by a xml request
// then it returns the slides for the carousel so that it can be displayed
function setWeather(weatherData){
    ...
    let weatherOutput = `
                <div class="forecast-item">
                    <h3 class="cardTime">${weatherData.name}, ${weatherData.sys.country}</h3>
                    <div class="cardImg"><img src="images/forecast-icons/${weatherData.weather[0].icon}.png" alt="Wind"></div>
                    <h4 class="cardTemp">${Math.round(weatherData.main.temp)}&#8451;</h4>
                    <h5 class="cardWeather">${weatherData.weather[0].main}</h5>
                    <p class="cardRain"><img src="images/precipitation.png" alt="Rain">${ifRaining(weatherData.rain)}mm</p>
                    <p class="cardHumidity"><img src="images/humidity.png" alt="Humidity">${weatherData.main.humidity}&#37;</p>
                    <p class="cardWind"><img src="images/wind-speed.png" alt="Wind Speed">${msToKMH(weatherData.wind.speed)} km/h</p>
                </div>
                `;
